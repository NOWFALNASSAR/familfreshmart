...
const BANK_MODES = ['upi', 'card', 'bank'];
const DENOMS = [500, 200, 100, 50, 20, 10, 5, 2, 1];
const CATS = ['Transport', 'Salary', 'Rent', 'Electricity', 'Loading', 'Packing', 'Maintenance', 'Petty cash', 'Other'];

const IN_TYPES = { sale: 1, customer_collection: 1, cash_in: 1, loan_in: 1, investment: 1 };
const LABEL = {
...
// Bank effect. Anything on UPI, card or bank transfer lands in the bank.
function bankEffect(t) {
  const a = Number(t.amount);
  if (t.is_setup || t.type === 'wastage') return 0;
  if (t.type === 'bank_deposit') return a;
  if (t.type === 'bank_withdraw') return -a;
  if (!BANK_MODES.includes(t.mode)) return 0;
  return IN_TYPES[t.type] ? a : -a;
}
...
export default function Page() {
  ...
  const { me, parties, entries, closings, settings, items, orders, bills, stock, users } = data;
  ...
  const bankBefore = live.filter((t) => t.date < date).reduce((a, t) => a + bankEffect(t), 0);
  const bankIn = dayTx.reduce((a, t) => a + Math.max(0, bankEffect(t)), 0);
  const bankOut = dayTx.reduce((a, t) => a + Math.max(0, -bankEffect(t)), 0);
  const bankBalance = bankBefore + bankIn - bankOut;

  const c = {
    data, me, parties, entries, live, closings, settings, items, orders, bills, stock, users,
    date, dayTx, closing, dayLocked, sales, purch, exp, waste, gp, gpRate,
    openingCash, cashIn, cashOut, expectedCash, bankBefore, bankIn, bankOut, bankBalance,
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="big">{money(c.bankBalance)}</div>
        <div className="rowb" style={{ marginTop: 10 }}><span className="k">Opening</span><span className="v">{money(c.bankBefore)}</span></div>
        <Row label="Bank in — UPI, card, deposit" value={c.bankIn} type="bank_in_all" colour="var(--leaf)" />
        <Row label="Bank out — payments, withdrawal" value={c.bankOut} type="bank_out_all" colour="var(--beet)" />
      </div>
...
function TxRow({ t, c }) {
  ...
  const ce = cashEffect(t), be = bankEffect(t);
  const net = ce || be;
  const where = ce ? 'cash' : be ? 'bank' : '—';
  return (
    <div className="item">
      <div>
...
function DayList({ c, type, close }) {
  const filters = {
    ...
    bank_in_all: (t) => bankEffect(t) > 0,
    bank_out_all: (t) => bankEffect(t) < 0,
  };
  const titles = {
    cash_in_all: 'Cash in', cash_out_all: 'Cash out',
    bank_in_all: 'Bank in', bank_out_all: 'Bank out',
  };
...
function DayClose(c) {
  ...
  const shouldRemain = c.expectedCash - dep;
  const diff = counted - shouldRemain;
  ...
  return (
    <>
      ...
      <div className="card">
        <span className="lbl">Cash to bank today</span>
        <input inputMode="decimal" value={deposit} placeholder="0" disabled={c.dayLocked}
          onChange={(e) => setDeposit(e.target.value)}
          style={{ fontFamily: 'var(--mono)', fontSize: 22, padding: '14px' }} />
        <div className="rowb" style={{ marginTop: 10 }}><span className="k">Banked</span>
          <span className="v">{money(dep)}</span></div>
        <div className="rowb"><b className="k" style={{ color: 'var(--chalk)' }}>Should remain in drawer</b>
          <b className="v" style={{ fontSize: 19 }}>{money(shouldRemain)}</b></div>
        <div className="rowb"><span className="k">Bank balance after this</span>
          <span className="v">{money(c.bankBalance + dep)}</span></div>
      </div>

      <div className="card">
        <span className="lbl">Count what is left</span>
        {DENOMS.map((d) => (
      ...
      {c.dayLocked ? (
        ...
        <button className="btn" style={{ marginTop: 14 }} disabled={counted === 0 && dep === 0}
          onClick={() => c.run('close', {
            date: c.date, opening: c.openingCash, expected: c.expectedCash,
            actual: counted, denoms: qty, deposit: dep,
          }, 'Day closed')}>
          Close {dshow(c.date)}
        </button>
      )}
      <p className="empty" style={{ fontSize: 12 }}>
        The counted cash becomes tomorrow's opening. The banked amount moves to the bank balance.
      </p>
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="rowb"><span className="k">Bank balance</span><span className="v">{money(c.bankBalance)}</span></div>
...
function Investment(c) {
  ...
  const stockValue = c.stock[0] ? Number(c.stock[0].value) : 0;
  const savings = c.expectedCash + c.bankBalance;
  ...
  return (
    <div className="card">
      ...
      <div className="rowb"><span className="k">Stock value{c.stock[0] ? ' · ' + dshow(c.stock[0].date) : ''}</span>
